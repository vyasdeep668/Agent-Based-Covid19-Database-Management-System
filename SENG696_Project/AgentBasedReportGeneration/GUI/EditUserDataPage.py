...
class EditUserDataPage(EditUserDataPageFrame):
    ...
    def Button7_Callback(self, controller):
        ...
        if not Application.DOB:
            tkinter.messagebox.showerror(title="Error", message="Please Enter DOB")
            return
        if not Application.Address:
            tkinter.messagebox.showerror(title="Error", message="Please Enter Address")
            return
        if not Application.Contact:
            tkinter.messagebox.showerror(title="Error", message="Please Enter Contact")
            return

        CommData = ':' + Application.Name + ':' + Application.HCNo + ':' + Application.DOB + ':' + Application.Address + ':' + Application.Contact
        ReceivedData = Webportal_Agent.RequestData(AgentCommunication.WebPortalAgentID,
                                                   AgentCommunication.SystemDatabaseAgentID,
                                                   AgentCommunication.UserDataUpdateCommandID,
                                                   AgentCommunication.SuccessAckID,
                                                   CommData)

        # No Message received from Slave Agent
        if not ReceivedData:
            tkinter.messagebox.showerror(title="Error", message="Database Admin Agent is not Responding!")
            return
        ...
        # If Data Update Error Received from Slave Agent
        if ReceivedData[AgentCommunication.ErrorCodeIndex] == AgentCommunication.DataUpdateFailedAckID:
            tkinter.messagebox.showerror(title="Error", message="User Data update failed!")
            return

        # No Error Found and User Update Success
        tkinter.messagebox.showinfo(title="Success", message="User Data Updated!")
    ...
    def Button8_Callback(self, controller):
        ...
        ReceivedData = Webportal_Agent.RequestData(AgentCommunication.WebPortalAgentID,
                                                   AgentCommunication.SystemDatabaseAgentID,
                                                   AgentCommunication.UserDataDeleteCommandID,
                                                   AgentCommunication.SuccessAckID,
                                                   CommData)
        ...
        # If Data Update Error Received from Slave Agent
        if ReceivedData[AgentCommunication.ErrorCodeIndex] == AgentCommunication.DataDeleteFailedAckID:
            tkinter.messagebox.showerror(title="Error", message="User Data Delete failed!")
            return

        # No Error Found and User Update Success
        tkinter.messagebox.showinfo(title="Success", message="User Data Deleted!")
    ...
    def Button9_Callback(self, controller):
        ...
        ReceivedData = Webportal_Agent.RequestData(AgentCommunication.WebPortalAgentID,
                                                   AgentCommunication.SystemDatabaseAgentID,
                                                   AgentCommunication.UserDataAddCommandID,
                                                   AgentCommunication.SuccessAckID,
                                                   CommData)
        ...
        if ReceivedData[AgentCommunication.ErrorCodeIndex] == AgentCommunication.DataAddFailedAckID:
            tkinter.messagebox.showerror(title="Error", message="User Already added")
            return

        # No Error Found and User Update Success
        tkinter.messagebox.showinfo(title="Success", message="User Data Added!")
    ...
    def SetEntryData(self):
        ...
        self.Entry3.insert(0, Application.DOB)
        self.Entry4.insert(0, Application.Address)
        self.Entry5.insert(0, Application.Contact)

    def ClearEntryData(self):
        self.Entry1.delete(0, len(Application.Name))
        self.Entry2.delete(0, len(Application.HCNo))
        self.Entry3.delete(0, len(Application.DOB))
        self.Entry4.delete(0, len(Application.Address))
        self.Entry5.delete(0, len(Application.Contact))
